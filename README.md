...
│   └── tokenManager.js   # Auto-refresh scheduler + token persistence
...
Facebook Page Access Tokens expire every 60 days. The bot handles this automatically:

- On startup, tokens are loaded from the Railway Volume (`/app/data/token-store.json`), falling back to env vars if the file doesn't exist
- 60 seconds after startup, the scheduler runs an initial refresh
- Every 7 days thereafter, both tokens are refreshed automatically
- Refreshed tokens are written back to the Volume and persist across redeploys

You should see this in Railway logs every 7 days:

```
[TokenManager] ✓ IML token refreshed successfully. Expires: <date>
[TokenManager] ✓ IMBB token refreshed successfully. Expires: <date>
```

### Token Recovery (when auto-refresh fails)

If tokens fully expire (e.g. the bot was down for 60+ days), auto-refresh can't save you — you need to generate fresh tokens manually. Follow these steps:

**1. Get fresh Page Tokens via the Graph API**

In the [Graph API Explorer](https://developers.facebook.com/tools/explorer):

1. Make sure your app is selected and all three permissions are checked:
   - `pages_read_engagement`
   - `pages_read_user_content`
   - `pages_manage_metadata`
2. Generate a User Token, then run:
   ```
   GET /me/accounts
   ```
3. The response contains a dedicated `access_token` for each page — copy both (IML and IMBB)

**2. Exchange both for long-lived tokens**

Paste these URLs in your browser (replace values):

```
https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=YOUR_APP_ID&client_secret=YOUR_APP_SECRET&fb_exchange_token=IML_PAGE_TOKEN
```

```
https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=YOUR_APP_ID&client_secret=YOUR_APP_SECRET&fb_exchange_token=IMBB_PAGE_TOKEN
```

Copy the `access_token` from each response.

**3. Verify they are Page Tokens**

In the Graph API Explorer, run:

```
GET /debug_token?input_token=YOUR_NEW_TOKEN
```

Confirm `"type": "PAGE"` in the response. If it says `"type": "USER"`, you used the wrong token — go back to step 1 and use the tokens from `/me/accounts`, not the Explorer's token field.

**4. Update Railway and redeploy**

- Go to Railway → Variables tab
- Update `FACEBOOK_IML_PAGE_ACCESS_TOKEN` and `FACEBOOK_IMBB_PAGE_ACCESS_TOKEN`
- Railway will redeploy automatically

**5. Re-subscribe both pages**

In the Graph API Explorer, for each page token:

- Set method to **POST**, URL to `/me/subscribed_apps`
- Add parameter: `subscribed_fields` = `feed`
- Hit Submit — confirm `{ "success": true }`
...
```
[TokenManager] ✓ IML token refreshed successfully. Expires: <60 days from now>
[TokenManager] ✓ IMBB token refreshed successfully. Expires: <60 days from now>
```

> ⚠️ If a post failed while the tokens were expired, Facebook won't retry it. You'll need to repost manually on the Facebook page to get it into Telegram.
...
- The bot auto-refreshes tokens every 7 days, but if they fully expire follow the **Token Recovery** section above.
